...
// Función para ingresar una opción válida (1 o 2)
...
int FuncionIngresarEleccion(int x ){
    ...
    do
    {
        ...
        if ((x !=1) && (x != 2))
        {
            std:: cout << "Opcion no valida, Ingresar de nuevo: ";
        }
...
int JuegoJavier(){
    ...
    std::cout << "Eres uno de los ultimos sobrevivientes en la ciudad infestada de zombies.\n";
    std::cout << "Tu objetivo es escapar de la ciudad y encontrar un refugio seguro.\n\n";

    // Primera elección
    std::cout << "Ves una casa abandonada y una tienda de armas.\n";
    std::cout << "Donde quieres ir? \n1 = Casa, 2 = Tienda de armas: "<<std::endl;

    choice = FuncionIngresarEleccion (choice);

    if (choice == 1) {
        std::cout << "\nEntras en la casa en busca de suministros.\n";
        std::cout << "Encuentras algo de comida y un botiquin de primeros auxilios.\n";
        std::cout << "De repente, oyes ruidos, parece que hay un zombie en la casa.\n";

        std::cout << "Quieres eliges?\nEsconderte(1) ----  Luchar(2): ";
        ...
        if (choice == 1) {  
            ...
            std::cout << "Luchas valientemente, pero el zombie te hiere.";// de cada 5 ataques zombies 3 personas terminan infectada, la formula es (los casos favorables / los casos totales), dando una probabilidad de infectare con un 60 %
            ...
            if (a == 1){
                ...
                return 1;
                    ...
                    // Segunda elección
            std::cout <<std::endl<<std::endl<< " Avanzas por las calles y ves un grupo de zombies bloqueando el camino principal.\n";
            std::cout << "Puedes intentar rodearlos o buscar otro camino.\n";
            ...
            if (choice == 1) {
                ...
                std::cout << "¡Sigues avanzando!\n";
            } else if (choice == 2) {
                std::cout << "\nEncuentras un camino alternativo, pero es mas largo y escuchas ruidos extraños.\n";
                std::cout << "De repente, un zombie aparece de la nada y te ataca.\n";// DE CADA 5 ATAQUES ZOMBIES  3 PERSONAS TERMINAN INFECTADAS
                a =FuncionProbabilidad(0.6);
                if (a == 1){
                   std::cout<< "\n\nHas sido infectado, y en pocos minutos te convertiras en zombie...";
                    return 1;
                }
                    std::cout << "Afortunadamente logras defenderte, pero estás herido.\n";}


                    // Tercera elección
                    std::cout << "\nFinalmente, llegas a la salida de la ciudad, pero está cerrada con una cerca alta.\n";
                    std::cout << "Ves un auto cerca que podrías usar para derribar la cerca o podrias intentar treparla.\n";
                    std::cout << "Que decides hacer? (1 = Usar el auto, 2 = Trepar la cerca): ";
                    choice = FuncionIngresarEleccion (choice);
                    ...
                    if (choice == 1) {
                        ...
                        std::cout << "Logras abrirte paso y escapar de la ciudad. Has sobrevivido al apocalipsis zombie!\n";
                        return 1;
                    } else if (choice == 2) {
                        std::cout << "\nIntentas trepar la cerca, \n";//De cada 10 personas que trepan una reja  2 se caen por los nervios a los zombies
                            ...
                            std::cout << "Logras escapar, pero estas herido levemente.\n";
                            std::cout << "A lo lejos logras ver un refugio militar, te miran a lo lejos y van corriendo a salvarte. Sobreviviste al apocalipsis zombie!\n";
                            }}
                        }
    ...
    std::cout << "Ahora estas mejor preparado para enfrentarte a los zombies.\n";

    // Segunda eleccion
    std::cout << "Al salir de la tienda, escuchas ruidos extranos. Puedes:\n";
    std::cout << " Investigar el ruido (1) ----  Ignorar el ruido y seguir adelante (2)\n";
    choice = FuncionIngresarEleccion(choice);

                if (choice == 1) {
                    std::cout << "\nTe acercas al ruido y descubres a un grupo de zombies.\n";
                    std::cout << "Decides usar tu pistola para defenderte.\n";// de cada 100 tiros la pistola se encasquilla con 3 disparos
                    a =FuncionProbabilidad(0.03);
                            ...
                            std::cout<< "\n\nHas sido infectado, y en pocos minutos te convertiras en zombie...";
                                return 1;
                                    ...
                                    std::cout<<"Lograste esquivar al zombie!!\n";
                                }
                            }
                            else{
                                std::cout << "¡Disparas y logras eliminar a algunos, pero mas vienen hacia ti!\n";
                            }
                    std::cout << "Tienes que decidir rapidamente:\n";
                    std::cout << "Correr hacia un callejon cercano (1) ---- Seguir disparando hasta que se acaben las balas (2)\n";
                    choice = FuncionIngresarEleccion(choice);

                    if (choice == 1) {
                        std::cout << "\nCorres hacia el callejon y logras esconderte. Los zombies pierden tu rastro.\n";
                        std::cout << "Te tomas un momento para recuperar el aliento y planear tu proximo movimiento.\n";
                    } else if (choice == 2) {
                     ...
                     std::cout << "Logras salir con vida, pero estas herido levemente y agotado.\n";
                } else if (choice == 2) {
                    std::cout << "\nDecides ignorar el ruido y seguir adelante.\n";
                    std::cout << "Sin embargo, te encuentras con un zombie que te ataca.\n";
                    ...
                    if (a == 1){
                        ...
                        if (a == 1){
                            ...
                            return 1;
                                ...
                                std::cout << "¡Disparas y logras eliminar al zombie";
                            }
                }
                ...
                std::cout << "Puedes:\n";
                std::cout << "Tomar el camino a la izquierda, que parece mas seguro (1) ---- ";
                std::cout << "Tomar el camino a la derecha, que parece mas peligroso pero mas corto (2)\n";
                choice = FuncionIngresarEleccion(choice);

                if (choice == 1) {
                        std::cout << "\nLlegas a una estacion de servicio abandonada. Hay comida y agua adentro, pero parece peligroso.\n";
                        std::cout << "Quieres entrar?  Si (1) ---- No (2): ";
                        choice = FuncionIngresarEleccion(choice);
                    ...
                    std::cout << "Pero te ves superado y tienes que escapar.\n";
                }

                std::cout << "Logras escapar, pero estás herido levemente.\n";
                std::cout << "A lo lejos logras ver un refugio militar, te miran a lo lejos y van corriendo a salvarte. Sobreviviste al apocalipsis zombie!\n";}
        return 1;
    }

void MenuPrincipal (int *menuPrincipal) {
    std::cout <<std::endl <<std::endl <<"-----Pleitechon-----" <<std::endl; 
    std::cout << "1. Modos de supervivencia zombie" <<std::endl; 
    std::cout << "2. Integrantes del Equipo" <<std::endl; 
    std::cout << "3. Salir" <<std::endl; 

    std::cout <<std::endl <<"Elige una opcion:" <<std::endl; 
...
void intergrantesDelEquipo (void) {
    std::cout << "-----INTEGRANTES-----" <<std::endl <<std::endl;
    std::cout << "Javier Alejandro Salamanca Reyes 00056824" <<std::endl;
    std::cout << "Jose Martin Luna Ramos 00308324" <<std::endl;
    std::cout << "German David Villalobos Vaquiz 00109224" <<std::endl;
    std::cout << " Ulises Edgardo Ramirez Cabrera 00023624" <<std::endl;

    int continuar; 
    std::cout <<std::endl <<std::endl << "Presiona [1] cuando termines de leer" <<std::endl; 
    std::cin >> continuar; 
...
void ModosZombies (int *modoszombies) {
    std::cout <<std::endl <<"-----SOLITARIO-----" <<std::endl; 
    std::cout << "1. Partida express" <<std::endl; 
    std::cout << "2. Partida Larga" <<std::endl;
    std::cout << "3. Volver al menu principal" <<std::endl;
    std::cout << "4. Salir del juego" <<std::endl; 

    std::cout <<std::endl <<"Elige una opcion:" <<std::endl; 
    std::cin >> *modoszombies;
    ...
    do {
        ...
        do {
            ...
            case 1: 
                ModosZombies (&modoszombies);
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                std::cout <<std::endl << "Hasta Pronto! Gracias por jugar :D" <<std::endl; 
                regresarMenuPrincipal = 0; 
                break;
                    default:
                        std::cout <<std::endl << "Ha ingesado una opcion de Menu Invalido. Porfavor vuelva a intentarlo" <<std::endl; 
                        break;
                    }
                break;
            case 2:
            ...
            case 3: 
                ...
                break;
            default:
                std::cout << "Ha ingresado una opcion de Menu invalido. Porfavor vuelva a intentarlo" <<std::endl;
                regresarMenuPrincipal = 0; 
                break;
            }
